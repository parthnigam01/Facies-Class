...
class Model_Finder:
    ...
    def __init__(self,file_object,logger_object):
        ...
        self.lgbm = LGBMClassifier(objective='multiclass')
        self.svm = SVC()
    ...
    def get_best_params_for_svc(self,train_x,train_y):
        ...
        try:
            ...
            # Creating an object of the Grid Search class
            self.grid= GridSearchCV(LGBMClassifier(objective='multiclass'),self.param_grid_lgbm, verbose=3,cv=5)
            # finding the best parameters
            self.grid.fit(train_x, train_y)
            ...
            # creating a new model with the best parameters
            self.lgbm = LGBMClassifier(learning_rate=self.learning_rate, num_leaves=self.num_leaves, n_estimators=self.n_estimators)
            # training the mew model
            self.lgbm.fit(train_x, train_y)
            self.logger_object.log(self.file_object,
                                   'LGBM best params: ' + str(
                                       self.grid.best_params_) + '. Exited the get_best_params_for_lgbm method of the Model_Finder class')
            return self.lgbm
        except Exception as e:
            self.logger_object.log(self.file_object,
                                   'Exception occured in get_best_params_for_lgbm method of the Model_Finder class. Exception message:  ' + str(
                                       e))
            self.logger_object.log(self.file_object,
                                   'LGBM Parameter tuning  failed. Exited the get_best_params_for_lgbm method of the Model_Finder class')
            raise Exception()
    def get_best_params_for_lgbm(self,train_x,train_y):

        """
                                        Method Name: get_best_params_for_lgbm
                                ...
                                """
        self.logger_object.log(self.file_object,
                               'Entered the get_best_params_for_lgbm method of the Model_Finder class')
        try:
            # initializing with different combination of parameters
